...
> V1.0 完结于 2026-06-04，61 个测试用例 100% 通过。
> V2.0 目标：沉浸式打标工作流、标签分组体系、学生端体验升级。
...
│   ├── services/
│   │   ├── aliyun_oss_storage.py     [MOD] 新增 get_thumbnail_url()
│   │   └── tag_group_service.py      [NEW] TagGroup CRUD 服务
│   ├── routes/
│   │   ├── admin.py                  [MOD] 新增批量学生 / 标签分组 / 图片标签编辑
│   │   └── student.py                [MOD] 新增缩略图 URL 生成
│   ├── schemas/
│   │   ├── admin.py                  [MOD] 新增 TagGroupCreate/Response, ImageTagUpdate
│   │   └── student.py                [MOD] 新增 ImageThumbnailResponse
│   └── migrations/
...
│   └── student.html                  [MOD] 多选工具栏 UI
├── docs/
│   ├── task-tracker.md               [APPEND] Phase 9–13
│   └── graduation-album-design-v2.md [THIS FILE]
└── tests/
    ├── test_tag_group.py             [NEW] 标签分组服务 + API 测试
    └── test_v2_admin_api.py          [NEW] V2 新增 API 集成测试
```
...
**架构决策**：缩略图 URL 通过 `x-oss-process=image/resize,w_400` 动态生成，无需额外存储或 Lambda 触发器。OSS 对 `image/resize` 的处理延迟 <100ms。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/admin/students/batch` | 批量添加学生（body: `{"names": "张三,李四,王五"}`），自动创建同名 Tag |
| **修改** POST | `/api/admin/students` | 创建学生时自动创建同名 Tag（移入"未分类"组） |
| GET | `/api/admin/tag-groups` | 标签分组列表 |
...
| **新增** PUT | `/api/admin/images/{id}/tags` | 编辑图片标签（body: `{"tag_ids": [1, 3, 5]}`），用于二次编辑弹窗 |
| **修改** GET | `/api/admin/images?tagged=true\|false` | 新增 `tagged` 查询参数：`false` 返回未打标图片（未处理池），`true` 返回已打标图片，缺省返回全部 |

### 学生路由增量 (`/api/student`)

| 方法 | 路径 | 说明 |
|------|------|------|
| **修改** GET | `/api/student/my-images` | 返回的 ImageResponse 新增 `thumbnail_url` 字段（利用 OSS `x-oss-process`） |
...
1. `pytest tests/ -v` — 确保 V1.0 61 个用例 + V2.0 新增用例全部通过
2. 数据库迁移回滚测试：从 V1.0 DB 快照执行迁移脚本，验证数据完整性
3. `uvicorn app.main:app --reload` 启动服务
4. 进入 `/admin` 验证打标工作台：上传 10 张 → 未处理池显示 10 → 点击标签逐张标注 → 池清空后 UI 隐藏
