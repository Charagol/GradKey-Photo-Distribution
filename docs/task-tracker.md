...
### Phase 27: V4.0 P6 — 容错增强 + P5 CSV 导出修复

**背景**: 应用在网络异常时静默失败。P6 为管理端和学生端增加基本的容错能力。

#### P5 遗留修复

- [x] CSV 导出首行加 UTF-8 BOM `\ufeff`（Excel 识别 UTF-8 编码）
- [x] 新增表头行：`"姓名","密钥"`
- [x] 测试更新：验证 BOM 存在、表头行正确

#### 任务 1: 上传失败自动重试（管理端）

- [x] `uploadImages()` — 网络错误（TypeError）指数退避重试：1s → 2s → 4s，最多 3 次
- [x] HTTP 错误（400/401/413）不重试，直接 toast
- [x] 重试中按钮显示「上传中（重试 N/3）...」+ 状态文字
- [x] 3 次耗尽后 toast「已重试 3 次，请检查网络后重试」
- [x] 每次重试重建 `FormData`（不可重用流式对象）

#### 任务 2: 网络状态监测与断网提示（管理端 + 学生端）

- [x] `initOfflineBanner()` — 监听 `window` `offline`/`online` 事件
- [x] 断网时页面顶部插入红色 fixed banner（z-40），可手动 × 关闭
- [x] 网络恢复时自动移除
- [x] 下次 offline 事件重新显示（即使之前手动关闭）
- [x] 管理端和学生端各自独立实现

#### 任务 3: OSS 不可达图片降级提示（管理端 + 学生端）

- [x] `window._imgLoadFailedReported` 全局标志位
- [x] `window.reportImageLoadFailure()` — 首次触发时 toast「图片加载失败，请检查网络或稍后刷新」
- [x] 同一页面生命周期仅 toast 一次（避免刷屏）
- [x] `<img onerror>` 中追加 `reportImageLoadFailure()` 调用
- [x] 现有灰色占位 SVG 保留不变

#### 测试覆盖

- [x] 113/113 全量测试通过（纯前端改动，后端仅 CSV 修复更新了测试断言）
